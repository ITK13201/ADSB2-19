...
#define ITERATE 3 // 読み取る行数
#define JUDGE 8   // 同一の箇所を読み取ったデータであると判定するハミング距離の閾値
#define OVER 100  // 1つのindexに重複できるデータの個数
#define WEIGHT 5  // 多数決決定法において当たりと判定されたものに付ける重み
...
typedef struct{
  int left[ITERATE];
  // int right[ITERATE]; // 最小となったときの相手のcount_subを入れる
  int ham[ITERATE];
...
int check(int index){
  ...
  int res =0;
  int search_cnt = 0;
  int iter_sub[OVER*3] = {-1};
  int count_sub[OVER*3] = {-1};
  for(int i=0; i<ITERATE; i++){
    ...
    if(tmp_flg == 1){ // 今見ているITERATEに当たりが1つでもあった場合
      for(int j=0; j<ITERATE; j++)
        if(check_sub[i].left[j] != -1){ // 当たったときの自分のcount数が判明したとき
          // flgをITERATEで管理したとき：block[i][index].flg[flg_cnt] = check_sub[i].left[j]; // blockにそのcount数を格納する
...
void dist_each_sub(int iterate, int index, int bit, unsigned char c, unsigned char data[]){
  int new_index = 0;
  id_char[bit] = c;
  new_index = hexa2num(id_char);
  if(new_index <= 5555){
    for(int i=0; i<18; i++){
      block[iterate][new_index].data[block[iterate][new_index].count][i] = data[i];
    }
    block[iterate][new_index].count++;
  }
  
}

/*dist_each: 分配指せる箇所のiterate, index, とbitの桁を指定し、分配を行う--------------------------------*/
void dist_each(int iterate, int index, int bit, unsigned char data[]){
  num2hexa(index);
  if(id_char[bit] == 'A'){
...
void distribute_sub(int index){
  ...
  for(int i=0; i<ITERATE; i++){ // 各iterateを探索
    for(int j=0; j<block[i][index].count; j++){ // 指定したindexの各count値が当たりであるか探索
      /*flg_check = 0;
      for(int k=0; k<ITERATE; k++){ // 各count値に対して, flgの内の1つと合致するか探索
        if(j == block[i][index].flg[k]){
          flg_check = 1; break;
        }
      }*/
      if(block[i][index].flg[j] == 1) continue; // 当たりであればスルー
      else{ // 外れであれば分配する必要あり
        for(int k=0; k<7; k++){ // データの分配
          dist_each(i, index, k, block[i][index].data[j]);
        }
      }
    }
  }
...
void Toblock_sub(int num, int kaisu, FILE *fp){
  ...
  int index = 0;
  int cum_count =0;
  if(num != 0 && kaisu == 0) dis = fgetc(fp); /*改行を退避させる*/
  for(int i=0; i<25; i++){
